...
class TorchVocab(object):
    """
    :property freqs: collections.Counter, コーパス中の単語の出現頻度を保持するオブジェクト持有一个词在语料库中的出现频率的对象。
    :property stoi: collections.defaultdict, string → id の対応を示す辞書 一本字典显示了以下内容的对应关系
    :property itos: collections.defaultdict, id → string の対応を示す辞書 一本字典显示了以下内容的对应关系
    """
    def __init__(self, counter, max_size=None, min_freq=1, specials=['<pad>', '<oov>'],
                 vectors=None, unk_init=None, vectors_cache=None):
        """
        :param counter: collections.Counter, データ中に含まれる単語の頻度を計測するためのcounter 衡量数据中单词的频率。
...
def main():
    ...
    parser.add_argument('--corpus_path', '-c', type=str, default='data_drug/chembl_30_corpus.txt', help='path to th ecorpus')
    parser.add_argument('--out_path', '-o', type=str, default='data_drug/vocab_30.pkl', help='output file')
