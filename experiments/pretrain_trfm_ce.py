...
def parse_arguments():
    ...
    parser.add_argument('--vocab', '-v', type=str, default='/home/ubuntu/projects/smiles_transformer/models/vocab_AH_20W_shuffle.pkl', help='vocabulary (.pkl)')
    parser.add_argument('--data', '-d', type=str, default='/home/ubuntu/projects/smiles_transformer/experiments/data/zinc_AH_20W_shuffle.csv', help='train corpus (.csv)')
...
def test_model():
    ...
    model = TrfmSeq2seq(len(vocab), args.hidden, len(vocab), args.n_layer).to('cuda')
    model.load_state_dict(torch.load('models/chem24_B8_n4_1_2000_good.pkl'))
    ...
    for b, sm in tqdm(enumerate(train_loader)):
        ...
        loss = F.nll_loss(output.view(-1, len(vocab)),sm.contiguous().view(-1), ignore_index=PAD)
        # loss = criterion(output.view(-1, len(vocab)),sm.contiguous().view(-1))
        #loss = evaluate(model, train_loader, vocab) 
        writer.add_scalar('loss', loss.item(), step)
        writer.add_graph(model,sm)
        step += 1

        # print('Train: iter {:5d} | loss {:.7f} | ppl {:.3f}'.format(b, loss, math.exp(loss)))
...
def main():
    ...
    #device = 'cpu'
    writer = SummaryWriter('/home/ubuntu/projects/smiles_transformer/tfevent/'+args.data.split(sep='/')[-1].split(sep='.')[0])
    ...
    model = TrfmSeq2seq(len(vocab), args.hidden, len(vocab), args.n_layer).to('cuda')
    #model.load_state_dict(torch.load('models/trfm_12_23000.pkl'))
    ...
    for e in range(0, args.n_epoch):
        for b, sm in tqdm(enumerate(train_loader)):
            ...
            if loss >= 1.8 and loss <= 3:
                print("loss in [1.8,3]")
                if not os.path.isdir(".save"):
                    os.makedirs(".save")
                torch.save(model.state_dict(), '.save/trfm_B%d_%d_%d.pkl' % (args.batch_size,e+1,b))
            

            if b%1==0:
                print('Train {:3d}: iter {:5d} | loss {:.7f} | ppl {:.3f}'.format(e, b, loss.item(), math.exp(loss.item())))
            if b%100==0:
                loss = evaluate(model, test_loader, vocab)
                print('Val {:3d}: iter {:5d} | loss {:.7f} | ppl {:.3f}'.format(e, b, loss, math.exp(loss)))
