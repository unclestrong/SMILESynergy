...
def parse_arguments():
    ...
    parser.add_argument('--vocab', '-v', type=str, default='models/vocab_30.pkl', help='vocabulary (.pkl)')
    parser.add_argument('--data', '-d', type=str, default='experiments/data/chembl_30.csv', help='train corpus (.csv)')
        ...
        for b, sm in tqdm(enumerate(train_loader)):
            ...
            if b%100==0:
                ...
                if not best_loss or loss < best_loss:
                    print("[!] saving model...")
                    if not os.path.isdir('.save/'+args.data.split(sep='/')[-1].split(sep='.')[0]):
                        os.makedirs('.save/'+args.data.split(sep='/')[-1].split(sep='.')[0])
                    torch.save(model.state_dict(), '.save/'+args.data.split(sep='/')[-1].split(sep='.')[0]+'/B%d_%d_%d.pkl' % (args.batch_size,e+1,b))
                    best_loss = loss
